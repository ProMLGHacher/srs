...
function sanitizeOneMediaSection(sectionLines: string[]): string[] {
  ...
  for (const [ssrc, key] of ssrcToKey) if (key === keepKey) keepSsrcs.add(ssrc)
  for (const line of sectionLines) {
    const m = line.match(/^a=ssrc-group:FID (\d+) (\d+)/)
    if (m && keepSsrcs.has(Number(m[1]))) keepSsrcs.add(Number(m[2]))
  }
  ...
  for (let idx = 1; idx < sectionLines.length; idx++) {
    ...
    if (fid) {
      const main = Number(fid[1]); const rtx = Number(fid[2])
      if (keepSsrcs.has(main) && keepSsrcs.has(rtx)) result.push(line)
      continue
    }
    const sim = line.match(/^a=ssrc-group:SIM (.+)/)
    if (sim) {
      const ids = sim[1].trim().split(/\s+/).map((x) => Number(x)).filter((n) => !Number.isNaN(n))
      const kept = ids.filter((id) => keepSsrcs.has(id))
      if (kept.length > 0) result.push(kept.length === ids.length ? line : `a=ssrc-group:SIM ${kept.join(" ")}`)
      continue
    }
    const msidLine = line.match(/^a=msid:(\S+) (\S+)/)
    if (msidLine) {
      if (`${msidLine[1]}|${msidLine[2]}` === keepKey) result.push(line)
      continue
    }
    result.push(line)
  }
